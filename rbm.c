...
// NOTE: "Positive" refers to values yielded from the actual data whereas
// "Negative" refers to values generated by simulating using the network

// vishid are the weights.
// Vishid[i][k][j] = W_ij^k (the weight between the softmax unit
// corresponding to movie i being rated k and hidden unit j)
double vishid[NMOVIES][SOFTMAX][TOTAL_FEATURES];

// Biases for each of the visible units 
// visbiases[i][k] is the bias for movie i and rating k (= b_i^k)
double visbiases[NMOVIES][SOFTMAX];

// Biases for each of the hidden units
// hidbiases[j] is the bias for hidden unit j (= b_j)
double hidbiases[TOTAL_FEATURES];

// Matrices to store the gradients (Contrastive Divergence Pos
// Neg, etc).
...
// Positive hidden state probabilities
// p(h_j = 1 | V)
double poshidprobs[TOTAL_FEATURES];

// Positive hidden state values, generated from probabilities
// in poshidprobs
char   poshidstates[TOTAL_FEATURES]; 

// Current positive hidden states ?!?!?!?
char   curposhidstates[TOTAL_FEATURES]; 

// Positive hidden state activations
double poshidact[TOTAL_FEATURES];
double neghidact[TOTAL_FEATURES];

// Negative hidden state probabilities and
// values
double neghidprobs[TOTAL_FEATURES];
char   neghidstates[TOTAL_FEATURES];

// Hidden state biases, inc.
double hidbiasinc[TOTAL_FEATURES];

// Copy of the negvisprobs matrix below
double nvp2[NMOVIES][SOFTMAX];

// Negative visible node ativation probabilities
double negvisprobs[NMOVIES][SOFTMAX];

// Negative visible softmax values (the actual
// ratings generated for each movie from the activation
// probabilities)
char   negvissoftmax[NMOVIES]; 

// Positive and negative visible node activations by movie/rating
...
// Number of times each movie was given
// a particular rating (2D array represented
// as a 1D array)
